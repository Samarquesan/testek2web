...
if (strpos($confirmacao, "audio/") === 0 || strpos($confirmacao, "video/") === 0) {
...
$formatoAceito = ["mp3", "wav", "wma", "mp4", "wmv", "mpg", "mov"];
...
if ($confirmadoEnvio == 0) {
    ...
    if (move_uploaded_file($_FILES["arquivo"]["tmp_name"], $pastaArquivo)) {
        if (!file_exists('arquivos.json')) {
            file_put_contents('arquivos.json', json_encode([]));
        }
